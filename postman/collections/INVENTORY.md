...
- **Root source:** [`mendableai/firecrawl`](https://github.com/mendableai/firecrawl)
  ...
  Upstream is **not** correct as published — see
  [the version split](#firecrawl-upstream-declares-the-wrong-server-for-two-operations). Push
  `postman/specs/firecrawl.openapi.json`, generated by `scripts/build-firecrawl-spec.py`.
- **Status (2026-08-08):** **in sync.** Spec in Postman matches the corrected artifact — 22 ops,
  both v2 overrides present. Collection carries all 22 operations.

Two caveats on this collection:

- **Five requests sit at the collection root** — `deep-research` ×2, `llmstxt` ×2, and
  `POST /feedback` — because no matching folder exists and the MCP server cannot create one.
  Create `research`, `llmstxt`, and `feedback` folders in the UI and drag them in.
- **The two v2-only requests hardcode `https://api.firecrawl.dev/v2`** instead of `{{baseUrl}}`,
  which is v1. This is deliberate and documented in each request's description. If `baseUrl` is
  ever moved to v2, these become wrong and the four v1-only operations break instead.
